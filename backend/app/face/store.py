...
def match_face(engine: FaceEngine, embedding: np.ndarray, threshold: float = 0.35):
    ...
    best = None
    best_d = 999.0
    # ensure incoming embedding is float32 and normalized
    try:
    ...
    for fid, name, blob in cur.fetchall():
        db_emb = _from_blob(blob).astype(np.float32)
        # normalize DB embedding to unit-length (defensive - stored data may not be exactly normalized)
        db_emb /= (np.linalg.norm(db_emb) + 1e-8)
        d = engine.cosine_distance(emb, db_emb)
        if d < best_d:
            best_d = d
